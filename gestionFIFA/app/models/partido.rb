class Partido < ApplicationRecord
  ...
  def marcador
    return 'Pendiente' unless finalizado?
  ...
  def equipos_distintos
    return unless equipo_local_id.present? && equipo_visitante_id.present? && equipo_local_id == equipo_visitante_id

    errors.add(:base, 'un equipo no puede jugar contra sí mismo')
  end

  def grupo_requerido_en_fase_de_grupos
    return unless fase == 'fase_grupos' && grupo.nil?
